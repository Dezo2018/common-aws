...
for page in response_iterator:
    ...
    for obj in objects:
        ...
        obj_name = obj['Key']
        get_logger(count).write(f'key={obj_name} ')

        dest_object = None

        try:
            dest_object = dest_client.head_object(
                Bucket=config['DEST_BUCKET_NAME'],
                Key=obj['Key']
            )

            # storage_class is none for standard storage class
            if dest_object.get('StorageClass') != object.get('StorageClass') and object.get('StorageClass') != None:
                get_logger(count).write(f'Transit from ST to ST-IA.\n')
        ...
        if dest_object == None:
            ...
            conf_obj = {'Metadata': {**object['Metadata'],
